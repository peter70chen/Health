...
export const TargetModal: React.FC<TargetModalProps> = ({
  ...
  return (
    <DialogShell
      ...
      overlayClassName="bg-black/80 z-[100] flex items-center justify-center p-4"
      panelClassName="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 w-full max-w-xs shadow-2xl"
    >
        <h3 id="target-dialog-title" className="text-white font-bold text-lg mb-4 flex items-center gap-2">
        ...
        <div className="relative">
          <label htmlFor="target-value" className="sr-only">
            {targetModal.type === 'daily' ? '每日攝取目標' : '每日運動目標'}
          </label>
          <input id="target-value" data-dialog-autofocus type="number" className="w-full bg-neutral-800 border border-neutral-700 rounded-xl p-4 text-2xl font-bold text-center text-white outline-none focus:border-teal-500 transition-colors" value={tempTargetValue} onChange={e => setTempTargetValue(e.target.value)} />
