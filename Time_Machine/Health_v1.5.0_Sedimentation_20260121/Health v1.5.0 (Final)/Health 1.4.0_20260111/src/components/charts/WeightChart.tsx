...
export const WeightChart: React.FC<WeightChartProps> = ({ data, targetWeight }) => {
    ...
    const ideals = data.map(d => d.ideal).filter(Boolean) as number[];
    const allW = [...weights, ...ideals];
    ...
    data.forEach((d, i) => {
        const x = getX(i);
        ideP += `${i === 0 ? 'M' : 'L'} ${x} ${getY_W(d.ideal || 0)} `;
        if (d.weight) {
    ...
    return (
        <div className="w-full h-64 relative border-l border-neutral-700 mt-4">
            <svg className="w-full h-full absolute inset-0" viewBox="0 0 100 115" preserveAspectRatio="none">
                ...
                {/* Ideal line */}
                {showIdeal && <path d={ideP} fill="none" stroke="#525252" strokeWidth="1" strokeDasharray="3" opacity="0.6" />}
            ...
            <div className="flex justify-center gap-4 absolute -bottom-8 w-full text-xs text-neutral-400 font-medium">
                ...
                <div
                    ...
                    className={`flex items-center gap-1 cursor-pointer select-none transition-opacity ${showIdeal ? 'opacity-100' : 'opacity-40'}`}
                >
                    <div className="w-2.5 h-2.5 rounded-full bg-neutral-600"></div>目標
                </div>
            </div>
        </div>
    );
