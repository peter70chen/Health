...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setResistanceLogs(safeLoadFromStorage(STORAGE_KEYS.RESISTANCE_LOGS, []));
  ...
  const dailyList = useMemo((): DailyListItem[] => {
    ...
    const waters = waterLogs.filter(l => l.date === currentViewDate).map(l => ({ ...l, _source: 'water' as const, type: 'water' as const }));

    // Add consolidated resistance log if exists
    const resLogs = resistanceLogs.filter(l => l.date === currentViewDate).reduce((acc, l) => acc + l.totalCalories, 0);
    const resItem = resLogs > 0 ? [{
      id: 999999, // specific ID for sorting? Or just make it distinct
      date: currentViewDate,
      type: 'activity' as const,
      activityName: '阻力運動 (總計)',
      activeCalories: resLogs,
      isManual: false,
      _source: 'activity' as const
    }] : [];

    return [...foods, ...activities, ...waters, ...resItem].sort((a, b) => b.id - a.id);
  }, [foodLogs, activityLogs, waterLogs, resistanceLogs, currentViewDate]);
  ...
  }, [foodLogs, activityLogs, waterLogs, today]);
  ...
  return (
    <div className="pb-32 max-w-md mx-auto min-h-screen relative bg-black">
      ...
      <div className="sticky top-0 bg-neutral-900 z-50 px-4 pt-[calc(env(safe-area-inset-top)+0.75rem)] pb-3 flex justify-between items-center shadow-md border-b border-neutral-800">
        <h1 className="text-xl font-bold text-teal-400 flex items-center gap-2"><Icons.Activity /> Health Plan <span className="text-xs text-neutral-500 font-normal mt-1">v1.4.0</span></h1>
          ...
          <>
            {isViewingHistory && (
              <div className="bg-yellow-900/30 border border-yellow-700 p-3 rounded-xl flex justify-between items-center animate-pulse relative mb-6">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setCurrentViewDate(today); setWaterLogs(safeLoadFromStorage(STORAGE_KEYS.WATER_LOGS, [])); }} className="bg-neutral-800 text-neutral-400 hover:text-white p-1.5 rounded-lg"><Icons.X className="w-4 h-4" /></button>
                  <button onClick={() => { setCurrentViewDate(today); setWaterLogs(safeLoadFromStorage(STORAGE_KEYS.WATER_LOGS, [])); }} className="bg-yellow-600 text-white text-xs px-3 py-1.5 rounded-lg font-bold">回到今日</button>
                </div>
              </div>
            )}
            ...
            <div className="bg-neutral-900 rounded-2xl shadow-sm border border-neutral-800 p-6">
              <div className="flex justify-between items-end mb-4 border-b border-neutral-800 pb-4">
                <div><div className={`text-5xl font-extrabold tracking-tight ${remaining < 0 ? 'text-red-500' : 'text-white'}`}>{remaining} <span className="text-2xl font-bold text-neutral-400">KCAL</span></div><div className="text-sm text-neutral-400 font-bold mt-1">今日剩餘額度</div><div className="text-xs text-neutral-500 mt-0.5">{currentViewDate.replace(/-/g, '年').replace(/年(\d{2})$/, '月$1日')}</div></div>
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                ...
                <div className="bg-teal-900/20 rounded-xl p-3 border border-teal-900/30 relative group">
                  <div className="text-sm text-teal-400 mb-1 font-bold">消耗 OUT</div><div className="text-2xl font-bold text-teal-500">-{dailyAct.cal + dailyRes.cal}</div>
                </div>
              </div>
                ...
                <div className="bg-neutral-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl space-y-5 mb-6 sm:mb-0 border border-neutral-800 max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="flex bg-neutral-800 p-1 rounded-xl">
                    ...
                    {inputModalType === 'activity' && (
                      <button onClick={() => setInputMethod('resistance')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${inputMethod === 'resistance' ? 'bg-neutral-700 text-teal-400 shadow-sm' : 'text-neutral-400'}`}>🏋️ 阻力</button>
                    )}
                    ...
                    <div className="space-y-4 animate-fadeIn">
                      ...
                      <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
                        {resistanceDefs.length === 0 ? (
                          ...
                          resistanceDefs.map(def => {
                            const activeItem = resistanceSession.find(i => i.defId === def.id);
                            const isChecked = !!activeItem;
                            return (
                              <div key={def.id} className={`bg-neutral-800 border ${isChecked ? 'border-teal-500/50' : 'border-neutral-700'} rounded-xl p-3 transition-colors`}>
                                ...
                                {isChecked && (
                                  <div className="grid grid-cols-4 gap-2 pl-7 animate-fadeIn">
